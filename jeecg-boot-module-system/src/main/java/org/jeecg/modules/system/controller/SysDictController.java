...
import org.jeecg.modules.tbFieldManager.service.ITbFieldManagerService;
...
public class SysDictController {
    ...
    public Result<List<DictModel>> loadDict(@PathVariable String dictCode, @RequestParam(name = "keyword") String keyword, @RequestParam(value = "sign", required = false) String sign, @RequestParam(value = "pageSize", required = false) Integer pageSize) {
    ...
    public Result<SysDict> delete(@RequestParam(name = "id") String id) {
        boolean haveUse = true;
        Result<SysDict> result = new Result<>();
        SysDict byId = sysDictService.getById(id);
        if (byId != null && Objects.nonNull(byId.getDictCode())) {
            haveUse = tbFieldManagerService.checkHaveDictUse(byId.getDictCode());
        }
        if (haveUse) {
            result.error500("该数据字典不存在，或该字典被标准字段引用，请先删除标准字段!");
        } else {
